...
function parseV38ShowPoker(payload:any):V38PokerState|null{
  ...
  const cardRoot=roots.find(x=>normalizeV38Result(x?.result)?.length)||{};
  ...
  const hasServerPoints=result.length>=10&&Number.isFinite(serverPlayer)&&Number.isFinite(serverBanker);
  const pointsAgree=fourCards&&(!hasServerPoints||(serverPlayer===calculatedPlayer&&serverBanker===calculatedBanker));
