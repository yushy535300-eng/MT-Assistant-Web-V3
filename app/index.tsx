...
export default function HomeScreen(){
  ...
  // Stop-loss reminder is independent from 今日輸贏. It only reads the official MT balance.
  const [currentBalance,setCurrentBalance]=useState<number|null>(null);
  const [stopLossOpen,setStopLossOpen]=useState(false);
  const [stopLossAlertOpen,setStopLossAlertOpen]=useState(false);
  const [stopLossPrincipal,setStopLossPrincipal]=useState(0);
  const [stopLossPercent,setStopLossPercent]=useState(20);
  const [stopLossEnabled,setStopLossEnabled]=useState(false);
  const stopLossAlertedRef=useRef(false);
  useEffect(()=>{
    if(typeof window==="undefined")return;
    try{const raw=window.localStorage.getItem("mt_stop_loss_v1");if(!raw)return;const v=JSON.parse(raw);if(Number(v?.principal)>0)setStopLossPrincipal(Number(v.principal));if(Number(v?.percent)>=1&&Number(v?.percent)<=90)setStopLossPercent(Number(v.percent));setStopLossEnabled(!!v?.enabled)}catch{}
  },[]);
  useEffect(()=>{
    if(!stopLossEnabled||!Number.isFinite(currentBalance as number)||stopLossPrincipal<=0||stopLossAlertedRef.current)return;
    const line=stopLossPrincipal*(1-stopLossPercent/100);
    if((currentBalance as number)<=line){stopLossAlertedRef.current=true;setStopLossAlertOpen(true)}
  },[currentBalance,stopLossEnabled,stopLossPrincipal,stopLossPercent]);
  const saveStopLoss=()=>{
    if(stopLossPrincipal<=0){notify("請設定本金");return}
    setStopLossEnabled(true);stopLossAlertedRef.current=false;setStopLossOpen(false);
    if(typeof window!=="undefined")try{window.localStorage.setItem("mt_stop_loss_v1",JSON.stringify({principal:stopLossPrincipal,percent:stopLossPercent,enabled:true}))}catch{}
  };
  ...
  const startConnection=(autoReason?:string,tokenSourceOverride?:string)=>{
    ...
    const requestBalance=()=>authenticated&&ws.readyState===WebSocket.OPEN&&ws.send(JSON.stringify({method:"POST",action:{name:"/api/v1/member/me/balance"}}));
    const startBalanceRefresh=()=>{requestBalance();if(balanceRefreshTimer)clearInterval(balanceRefreshTimer);balanceRefreshTimer=setInterval(()=>{if(isCurrentSocket()&&authenticated)requestBalance()},5000)};
    ...
    ws.onmessage=e=>{if(!isCurrentSocket())return;try{
      ...
      if(name.includes("/show_poker")){
        ...
        if(name.includes("/api/v1/member/me/balance")){
          const points=Number(p?.msg?.user?.points??p?.body?.user?.points??p?.data?.user?.points);
          if(Number.isFinite(points))setCurrentBalance(points);
          return;
        }
  ...
  const FloatingAssistant=({insideMt=false}:{insideMt?:boolean})=>{
    ...
    const page1=<View {...pageSwipe.panHandlers} style={s.assistPage}>
      ...
      <View style={s.todayPnlBox}><Text style={s.smallLabel}>今日輸贏</Text><View style={s.todayPnlRight}><Text style={[s.todayPnlValue,{color:todayPnl===null?"#FFFFFF":todayPnl>0?"#4ED58B":todayPnl<0?"#FF6973":"#FFFFFF"}]}>{todayPnl===null?"—":`${todayPnl>0?"+":""}${todayPnl.toLocaleString()}`}</Text><Pressable style={s.stopLossMiniBtn} onPress={()=>{if(stopLossPrincipal<=0&&currentBalance!==null)setStopLossPrincipal(Math.max(0,currentBalance));setStopLossOpen(true)}}><Text style={s.stopLossMiniText}>{stopLossEnabled?`止損 ${stopLossPercent}%`:"止損設定"}</Text></Pressable></View></View>
  ...
  const TerminalParityModel=({insideMt=false}:{insideMt?:boolean})=>{
    ...
    const status=!connected?"未連線":!data?"等待牌面":ready?"已完成":"發牌中";
  ...
  return <ScreenContainer edges={["top","left","right","bottom"]} containerClassName="bg-[#080E17]" className="bg-[#080E17]">
    <View style={[s.screen,desktop&&Platform.OS==="web"?s.screenDesktopZoom:null]}>
      ...
      <Modal visible={stopLossOpen} transparent animationType="fade" onRequestClose={()=>setStopLossOpen(false)}><View style={s.modalShade}><View style={s.stopLossModal}><View style={s.modalHead}><Text style={s.stopLossTitle}>🛡 止損設定</Text><Pressable onPress={()=>setStopLossOpen(false)}><MaterialIcons name="close" size={19} color="#fff"/></Pressable></View><View style={s.stopLossLine}><Text style={s.stopLossLabel}>目前餘額</Text><Text style={s.stopLossValue}>{currentBalance===null?"讀取中…":currentBalance.toLocaleString()}</Text></View><View style={s.stopLossLine}><Text style={s.stopLossLabel}>本金</Text><TextInput value={stopLossPrincipal>0?String(Math.round(stopLossPrincipal)):""} onChangeText={v=>setStopLossPrincipal(Math.max(0,Number(v.replace(/[^0-9.]/g,""))||0))} keyboardType="decimal-pad" placeholder="輸入本金" placeholderTextColor="#647A8A" style={s.stopLossInput}/></View><Pressable style={s.useBalanceBtn} disabled={currentBalance===null} onPress={()=>currentBalance!==null&&setStopLossPrincipal(currentBalance)}><Text style={s.useBalanceText}>使用目前餘額</Text></Pressable><View style={s.stopLossLine}><Text style={s.stopLossLabel}>止損比例</Text><View style={s.percentRow}><Pressable style={s.percentBtn} onPress={()=>setStopLossPercent(v=>Math.max(5,v-5))}><Text style={s.percentBtnText}>−</Text></Pressable><Text style={s.percentValue}>{stopLossPercent}%</Text><Pressable style={s.percentBtn} onPress={()=>setStopLossPercent(v=>Math.min(90,v+5))}><Text style={s.percentBtnText}>＋</Text></Pressable></View></View><View style={s.stopLossLine}><Text style={s.stopLossLabel}>警戒線</Text><Text style={s.stopLossWarn}>{stopLossPrincipal>0?Math.round(stopLossPrincipal*(1-stopLossPercent/100)).toLocaleString():"—"}</Text></View><Pressable style={s.stopLossSave} onPress={saveStopLoss}><Text style={s.stopLossSaveText}>開啟提醒</Text></Pressable></View></View></Modal>
      <Modal visible={stopLossAlertOpen} transparent animationType="fade" onRequestClose={()=>setStopLossAlertOpen(false)}><View style={s.modalShade}><View style={s.stopLossAlert}><Text style={s.stopLossAlertIcon}>⚠️</Text><Text style={s.stopLossAlertTitle}>止損提醒</Text><Text style={s.stopLossAlertText}>已達設定的止損警戒線{`\n`}建議適度休息，理性遊戲。</Text><Pressable style={s.stopLossSave} onPress={()=>setStopLossAlertOpen(false)}><Text style={s.stopLossSaveText}>我知道了</Text></Pressable></View></View></Modal>
...
const s=StyleSheet.create({
  ...
  modalShade:{flex:1,backgroundColor:"rgba(0,0,0,.72)",alignItems:"center",justifyContent:"center",padding:16},connectionModal:{width:"100%",maxWidth:760,maxHeight:"92%",backgroundColor:"#162231",borderWidth:1,borderColor:"#31506A",borderRadius:8,padding:18},smallModal:{width:"100%",maxWidth:520,backgroundColor:"#162231",borderWidth:1,borderColor:"#31506A",borderRadius:8,padding:18},stopLossModal:{width:"86%",maxWidth:310,backgroundColor:"#101D29",borderWidth:1,borderColor:"#315D79",borderRadius:9,padding:12},stopLossTitle:{color:"#fff",fontSize:14,fontWeight:"900"},stopLossLine:{minHeight:34,flexDirection:"row",alignItems:"center",justifyContent:"space-between",gap:8},stopLossLabel:{color:"#AFC1CD",fontSize:9,fontWeight:"800"},stopLossValue:{color:"#fff",fontSize:12,fontWeight:"900"},stopLossInput:{width:126,height:30,borderWidth:1,borderColor:"#36536A",borderRadius:5,backgroundColor:"#08131D",color:"#fff",paddingHorizontal:8,fontSize:11,fontWeight:"800",textAlign:"right"},useBalanceBtn:{alignSelf:"flex-end",height:24,paddingHorizontal:8,borderRadius:4,backgroundColor:"#172F43",alignItems:"center",justifyContent:"center",marginBottom:3},useBalanceText:{color:"#8FD3FF",fontSize:8,fontWeight:"900"},percentRow:{flexDirection:"row",alignItems:"center",gap:8},percentBtn:{width:25,height:25,borderRadius:4,backgroundColor:"#214A70",alignItems:"center",justifyContent:"center"},percentBtnText:{color:"#fff",fontSize:15,fontWeight:"900"},percentValue:{minWidth:38,textAlign:"center",color:"#fff",fontSize:12,fontWeight:"900"},stopLossWarn:{color:"#FFCF66",fontSize:12,fontWeight:"900"},stopLossSave:{height:32,borderRadius:5,backgroundColor:"#214A70",alignItems:"center",justifyContent:"center",marginTop:7},stopLossSaveText:{color:"#fff",fontSize:10,fontWeight:"900"},stopLossAlert:{width:"82%",maxWidth:280,backgroundColor:"#101D29",borderWidth:1,borderColor:"#7B5C32",borderRadius:9,padding:15,alignItems:"center"},stopLossAlertIcon:{fontSize:23},stopLossAlertTitle:{color:"#fff",fontSize:15,fontWeight:"900",marginTop:4},stopLossAlertText:{color:"#CBD7DF",fontSize:10,lineHeight:16,textAlign:"center",marginTop:7,marginBottom:3},modalHead:{flexDirection:"row",justifyContent:"space-between",alignItems:"center",marginBottom:12},modalTitle:{color:"#fff",fontSize:17,fontWeight:"800"},modalNote:{color:"#BAC7D0",fontSize:10,lineHeight:15,backgroundColor:"#0C1721",padding:10,borderRadius:5,marginBottom:12},fieldLabel:{color:"#C6D3DC",fontSize:10,marginBottom:5,marginTop:8},modalInput:{height:42,borderWidth:1,borderColor:"#36536A",borderRadius:5,backgroundColor:"#08131D",color:"#fff",paddingHorizontal:10},mappingRow:{flexDirection:"row",gap:6,marginTop:10,flexWrap:"wrap"},mapChip:{color:"#C8D4DD",fontSize:9,backgroundColor:"#263A4C",paddingHorizontal:8,paddingVertical:6,borderRadius:4},modalActions:{flexDirection:"row",gap:7,marginTop:12,flexWrap:"wrap"},actionBtn:{height:38,paddingHorizontal:12,borderRadius:5,justifyContent:"center"},btnText:{color:"#fff",fontWeight:"900",fontSize:10},syncText:{color:"#AFC0CB",fontSize:9,marginTop:11},logBox:{height:130,backgroundColor:"#08131D",borderRadius:5,padding:9,marginTop:4},logText:{color:"#B8C8D2",fontSize:8,lineHeight:13},helpText:{color:"#D2DDE4",fontSize:11,lineHeight:18},
