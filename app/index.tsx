...
function confidenceGlow(percent:number) {
  if(percent>=80)return "#E7B94D";   // champagne gold
  if(percent>=50)return "#63C7FF";   // ice blue
  return "#FFFFFF";                  // white below 50
}
...
function MatrixMark({size=28}:{size?:number}){
  return <View style={[s.matrixMark,{width:size,height:size,borderRadius:Math.max(7,size*.24)}]}>
    <View style={s.matrixMarkGrid}>
      <View style={[s.matrixBar,{left:"18%",transform:[{rotate:"-18deg"}]}]}/><View style={[s.matrixBar,{left:"39%",transform:[{rotate:"18deg"}]}]}/><View style={[s.matrixBar,{right:"18%",transform:[{rotate:"-18deg"}]}]}/>
    </View><Text style={[s.matrixMarkText,{fontSize:Math.max(9,size*.27)}]}>MT</Text>
  </View>
}
...
export default function HomeScreen(){
  ...
  const panelBaseWidth=desktop?560:tablet?Math.min(520,width*0.90):Math.max(310,width*0.88);
  ...
  const assistConfidenceColor=confidenceGlow(assistConfidence);
  ...
  const MultiTableRadar=({insideMt=false}:{insideMt?:boolean})=>{
    const ordered=bestRadar?[bestRadar,...radarSignals.filter(x=>x.id!==bestRadar.id)]:radarSignals;
    if(!radarOpen)return <Pressable onPress={()=>setRadarOpen(true)} style={[s.radarLauncher,insideMt&&s.radarLauncherMt,!desktop&&s.radarLauncherMobile]}><MaterialIcons name="radar" size={15} color="#E7B94D"/><Text style={s.radarLauncherText}>多桌雷達</Text>{bestRadar?<Text style={[s.radarLauncherBest,{color:resultColor(bestRadar.decision.side)}]}>{bestRadar.id} · {bestRadar.decision.side} · {bestRadar.confidence}%</Text>:null}</Pressable>;
    return <View style={[s.radarPanel,insideMt&&s.radarPanelMt,!desktop&&s.radarPanelMobile]}>
      <View style={s.radarHead}><View><Text style={s.radarKicker}>MT MATRIX · MULTI-TABLE RADAR</Text><View style={s.radarTitleRow}><Text style={s.radarTitle}>多桌雷達</Text>{bestRadar?<Text style={s.radarBest}>👑 首選 {bestRadar.id} · {bestRadar.decision.side} · {bestRadar.confidence}%</Text>:<Text style={s.radarWaiting}>等待有效牌路</Text>}</View></View><Pressable onPress={()=>setRadarOpen(false)} style={s.radarClose}><MaterialIcons name="keyboard-arrow-up" size={20} color="#FFF2C2"/></Pressable></View>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.radarRail}>{ordered.map(item=>{
        const best=item.id===bestRadar?.id; const c=confidenceGlow(item.confidence);
        return <Pressable key={item.id} onPress={()=>{setAssistTableId(item.id);setRadarDetailId(item.id)}} style={[s.radarCard,best&&s.radarCardBest]}>
          <View style={s.radarCardTop}><Text style={s.radarRoom}>{item.id}</Text>{best?<Text style={s.radarPick}>首選</Text>:null}</View>
          <View style={s.radarCardMain}><Text style={[s.radarSide,{color:item.ready?resultColor(item.decision.side):"#7B8B96"}]}>{item.ready?item.decision.side:"等待"}</Text><View style={s.radarConfidence}><MaterialIcons name="lightbulb" size={12} color={item.ready?c:"#687680"} style={item.ready?({textShadowColor:c,textShadowRadius:9} as any):undefined}/><Text style={[s.radarConfidenceText,{color:item.ready?c:"#687680"}]}>{item.ready?`${item.confidence}%`:"—"}</Text></View></View>
          <Text numberOfLines={1} style={s.radarPattern}>{item.ready?detectPattern(item.table.results):"等待牌路資料"}</Text>
  ...
  const FloatingAssistant=({insideMt=false}:{insideMt?:boolean})=>{
    ...
    return <Animated.View onLayout={(e:any)=>{const l=e.nativeEvent?.layout;if(l?.width&&l?.height){panelSizeRef.current={width:l.width,height:l.height}}}} style={[s.floatPanel,!desktop?s.floatPanelMobile:null,{width:panelBaseWidth,maxWidth:width-20},insideMt?s.floatPanelMt:null,Platform.OS==="web"?({overscrollBehavior:"contain"} as any):null,{transform:panelPosition.getTranslateTransform()}]}>
      ...
      {assistPage===0?page1:assistPage===1?page2:page3}<View style={[s.pageDots,!desktop?s.pageDotsMobile:null]}>{[0,1,2].map(i=><Pressable key={i} onPress={()=>setAssistPage(i)}><View style={[s.pageDot,assistPage===i&&s.pageDotActive]}/></Pressable>)}</View>
    </Animated.View>;
  };
...
const s=StyleSheet.create({
  ...
  recommendMetaRow:{flexDirection:"row",alignItems:"center",justifyContent:"space-between",gap:3,marginTop:3},recommendStrategyMeta:{marginTop:0,flexShrink:1},confidenceInline:{flexDirection:"row",alignItems:"center",gap:2,flexShrink:0},confidenceText:{fontSize:8,fontWeight:"900",textShadowRadius:7},
  radarLauncher:{position:"absolute",top:66,right:14,zIndex:90,height:30,paddingHorizontal:9,borderRadius:7,borderWidth:1,borderColor:"#9E7931",backgroundColor:"rgba(14,17,19,.96)",flexDirection:"row",alignItems:"center",gap:5,shadowColor:"#E7B94D",shadowOpacity:.18,shadowRadius:7,elevation:18},radarLauncherMt:{zIndex:9998},radarLauncherMobile:{top:61,right:8,maxWidth:"78%"},radarLauncherText:{color:"#FFF2C2",fontSize:9,fontWeight:"900"},radarLauncherBest:{fontSize:8,fontWeight:"900",marginLeft:3},
  radarPanel:{position:"absolute",top:66,left:14,right:14,zIndex:90,height:92,borderRadius:9,borderWidth:1,borderColor:"#9E7931",backgroundColor:"rgba(6,15,22,.975)",paddingHorizontal:8,paddingVertical:6,shadowColor:"#E7B94D",shadowOpacity:.16,shadowRadius:10,elevation:18},radarPanelMt:{zIndex:9998},radarPanelMobile:{top:61,left:7,right:7,height:88},radarHead:{height:25,flexDirection:"row",alignItems:"center",justifyContent:"space-between"},radarKicker:{color:"#B99546",fontSize:6.5,fontWeight:"900",letterSpacing:1},radarTitleRow:{flexDirection:"row",alignItems:"center",gap:8},radarTitle:{color:"#FFF2C2",fontSize:11,fontWeight:"900"},radarBest:{color:"#E7B94D",fontSize:8,fontWeight:"900"},radarWaiting:{color:"#82929D",fontSize:8,fontWeight:"800"},radarClose:{width:26,height:24,borderRadius:5,borderWidth:1,borderColor:"#725A29",backgroundColor:"#17140D",alignItems:"center",justifyContent:"center"},radarRail:{gap:5,paddingRight:4,alignItems:"center"},radarCard:{width:94,height:52,borderRadius:6,borderWidth:1,borderColor:"#31495A",backgroundColor:"#0B1924",paddingHorizontal:6,paddingVertical:4},radarCardBest:{width:104,borderColor:"#E7B94D",backgroundColor:"#19170F",shadowColor:"#E7B94D",shadowOpacity:.28,shadowRadius:7,elevation:6},radarCardTop:{flexDirection:"row",alignItems:"center",justifyContent:"space-between"},radarRoom:{color:"#E8EEF2",fontSize:8,fontWeight:"900"},radarPick:{color:"#1A1305",backgroundColor:"#E7B94D",borderRadius:3,paddingHorizontal:3,paddingVertical:1,fontSize:5.5,fontWeight:"900"},radarCardMain:{flexDirection:"row",alignItems:"center",justifyContent:"space-between",marginTop:2},radarSide:{fontSize:12,fontWeight:"900"},radarConfidence:{flexDirection:"row",alignItems:"center",gap:1},radarConfidenceText:{fontSize:8,fontWeight:"900"},radarPattern:{color:"#91A6B4",fontSize:6.5,fontWeight:"800",marginTop:1},
  radarDetailModal:{width:"96%",maxWidth:900,backgroundColor:"#0A1721",borderWidth:1,borderColor:"#B78A35",borderRadius:10,padding:12},radarDetailTitle:{color:"#FFF2C2",fontSize:16,fontWeight:"900",marginTop:2},radarDetailStats:{flexDirection:"row",gap:6,marginBottom:8,flexWrap:"wrap"},radarDetailStat:{flexGrow:1,minWidth:90,backgroundColor:"#102335",borderWidth:1,borderColor:"#294B64",borderRadius:6,paddingHorizontal:8,paddingVertical:6},radarDetailLabel:{color:"#AFC1CD",fontSize:7,fontWeight:"800"},radarDetailValue:{color:"#F3F7F9",fontSize:11,fontWeight:"900",marginTop:2},radarDetailConfidence:{flexDirection:"row",alignItems:"center",gap:3,marginTop:2},radarRoadWrap:{backgroundColor:"#07131D",borderRadius:7,overflow:"hidden",borderWidth:1,borderColor:"#9E7931"},radarDetailNote:{color:"#C6D2DB",fontSize:9,lineHeight:14,marginTop:8},
  matrixMark:{backgroundColor:"#120E06",borderWidth:1,borderColor:"#D7A93F",alignItems:"center",justifyContent:"center",overflow:"hidden",shadowColor:"#E7B94D",shadowOpacity:.30,shadowRadius:5,elevation:3},matrixMarkGrid:{...StyleSheet.absoluteFillObject,opacity:.7},matrixBar:{position:"absolute",top:"13%",width:"15%",height:"61%",borderRadius:2,backgroundColor:"#DDB64F"},matrixMarkText:{position:"absolute",bottom:"5%",color:"#FFF2C2",fontWeight:"900",letterSpacing:-.7},threadsSignature:{flexDirection:"row",alignItems:"center",gap:4},threadsGlyph:{color:"#F2F8FC",fontSize:11,fontWeight:"900",borderWidth:1,borderColor:"#557487",borderRadius:8,width:16,height:16,lineHeight:14,textAlign:"center"},threadsId:{color:"#D9E3EA",fontSize:10.5,fontWeight:"900",letterSpacing:.15},threadsSignatureMobile:{marginTop:1,gap:3},threadsWord:{color:"#F2F8FC",fontSize:8.5,fontWeight:"900",letterSpacing:.15},threadsIdMobile:{fontSize:9.5},
  floatPanelMobile:{right:58,bottom:18,borderRadius:8},floatHeaderMobile:{height:28,paddingHorizontal:6},selectorWrapMobile:{marginHorizontal:5,marginTop:4},selectorMobile:{height:28,paddingHorizontal:7},assistPageMobile:{paddingHorizontal:5,paddingTop:4,paddingBottom:2,minHeight:96},decisionRowMobile:{gap:4},decisionBoxMobile:{minHeight:46,paddingHorizontal:5,paddingVertical:4},todayPnlBoxMobile:{marginTop:3,paddingHorizontal:7,paddingVertical:3},aiBoxMobile:{marginTop:3,paddingHorizontal:5,paddingVertical:4},pageDotsMobile:{height:12},
