...
export default function HomeScreen(){
  ...
  const panelMobileTargetWidth=Math.min(width*0.74,380);
  const panelMobileScale=Math.min(1,panelMobileTargetWidth/panelDesktopWidth); // fixed desktop canvas; whole panel/fonts/buttons scale together
  ...
  const FloatingAssistant=({insideMt=false}:{insideMt?:boolean})=>{
    ...
    return <Animated.View onLayout={(e:any)=>{const l=e.nativeEvent?.layout;if(l?.width&&l?.height){panelSizeRef.current={width:l.width,height:l.height}}}} style={[s.floatPanel,!desktop?s.floatPanelMobile:null,{width:panelBaseWidth},insideMt?s.floatPanelMt:null,Platform.OS==="web"?({overscrollBehavior:"contain",...(!desktop?{zoom:panelMobileScale}: {})} as any):null,{transform:Platform.OS!=="web"&&!desktop?[...panelPosition.getTranslateTransform(),{scale:panelMobileScale}]:panelPosition.getTranslateTransform()}]}>
