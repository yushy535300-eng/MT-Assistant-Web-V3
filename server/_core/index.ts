...
async function startServer() {
  ...
  app.post("/api/admin/whitelist-form",requireAdmin,async(req,res)=>{try{const u=String(req.body?.username||"").trim();if(!u)return adminRedirect(res,"請輸入平台帳號");const platform=String(req.body?.platform||"TZ").toUpperCase()==="OFA"?"OFA":"TZ";const d=String(req.body?.days||"permanent");await upsertWhitelist({platform,username:u,permanent:d==="permanent",days:d==="permanent"?null:Number(d),note:String(req.body?.note||"")});adminRedirect(res,`${platform}｜${u} 已新增並立即生效`)}catch(e:any){adminRedirect(res,`新增失敗：${e?.message||e}`)}});
