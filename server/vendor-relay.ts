...
export function abRoad(code:any):Road|null{
  ...
  // AB's first digit is the authoritative result marker. Digits 2/3 also
  // carry both final points, but comparing them alone loses special encodings.
  if(s[0]==="1")return"莊";if(s[0]==="2")return"閒";if(s[0]==="0")return"和";
  const banker=n(s[1]),player=n(s[2]);return banker===player?"和":banker>player?"莊":"閒";
}
...
class VendorRelay{
  ...
  private handleAb(o:any){
    ...
    if(cmd==="getGameHall"&&Array.isArray(p.D)){
      for(const r of p.D){
        ...
        const id=text(r?.BB,r?.AA);const roads=(Array.isArray(r?.WW3)?r.WW3.flat(4):[]).map(abRoad).filter(Boolean) as Road[];const c=count(roads);
        this.map.set(String(r.AA),{id,apiId:id,game:"百家樂",name:text(r?.II,"—"),players:"—",countdown:n(r?.HH?.BB),countdownUpdatedAt:Date.now(),roomId:id,tableBadge:String(r?.AA),shoe:text(r?.HH?.CC,"—"),round:roads.length,banker:c.莊,player:c.閒,tie:c.和,results:roads,trend:"",live:true,streamUrl:r?.Z16?String(r.Z16):undefined,lastUpdated:Date.now(),category:cat});
      }this.emit();return;
    }
    ...
    if(cmd==="pushGameTableResults"&&Array.isArray(p.G)){
      const prev=this.map.get(String(p.A));if(!prev)return;
      const roads=p.G.flat(4).map(abRoad).filter(Boolean) as Road[];const c=count(roads);
      this.map.set(String(p.A),{...prev,results:roads,round:n(p.C)||roads.length,banker:c.莊,player:c.閒,tie:c.和,lastResultKey:`${p.C||roads.length}:${roads.at(-1)||""}`,lastUpdated:Date.now()});
      this.emit();return;
    }
  ...
  private handleDb(root:any){
    ...
    const visit=(v:any,depth=0)=>{
      ...
      const gameId=n(v.gameId??v.game_id??v.gameTypeId??v.gameType??v.game_type);
      const id=text(v.fms,v.tableCode,v.table_code,v.tableName,v.table_name,v.tableId,v.table_id);
      let roads=v.roads??v.roadmaps??v.roadMap??v.roadData??v.roadList??v.beadRoad??v.beadPlateRoad??v.results??v.resultList??v.gameResultList;
      if(!Array.isArray(roads)){
        const hit=Object.entries(v).find(([k,x])=>/(?:road|result)/i.test(k)&&Array.isArray(x));
        if(hit)roads=hit[1];
      }
      if(id&&Array.isArray(roads)){
        const old=this.map.get(id);const rr=roads.flat(5).map((x:any)=>{
          const z=String(x?.result??x?.winner??x?.winType??x?.gameResult??x?.resultType??x?.code??x).toLowerCase();
          if(z.includes("bank")||z==="1"||z==="莊")return"莊";if(z.includes("play")||z==="2"||z==="閒")return"閒";if(z.includes("tie")||z==="3"||z==="和")return"和";
          const bp=n(x?.bankerPoint??x?.banker_point??x?.bankerScore),pp=n(x?.playerPoint??x?.player_point??x?.playerScore);
          if(bp||pp)return bp===pp?"和":bp>pp?"莊":"閒";return null;
        }).filter(Boolean) as Road[];const c=count(rr);
        if(!rr.length){if(Array.isArray(v))for(const x of v.slice(0,500))visit(x,depth+1);else for(const x of Object.values(v).slice(0,200))visit(x,depth+1);return}
